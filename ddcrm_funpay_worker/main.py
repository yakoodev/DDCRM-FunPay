...
from .cardinal_adapter import CardinalAdapter
...
adapter = CardinalAdapter(config=config, storage=storage)

app = FastAPI(
    title="DDCRM FunPay Worker",
    version="0.1.0",
    docs_url=None,
    redoc_url=None,
)
...
async def worker_product_create(request: Request, body: dict[str, Any]):
    return _idempotent(
        request=request,
        scope="worker:products:create",
        operation=lambda: _build_product_create_response(request, body),
    )
...
async def worker_product_update(request: Request, product_id: str, body: WorkerV2ProductUpdateRequest):
    ...
    return _idempotent(
        ...
        operation=lambda: _build_product_update_response(request, product_id, body),
    )
